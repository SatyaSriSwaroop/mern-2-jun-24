...
const searchBlogs = async(req,res) => {
    ...
    try{
        ...
        // const reqBlog = await Blog.find({title : title}); //same as above line
        return res.send(reqBlogs);
    }
    catch(error){
        res.status(500).send({message: "Oops! Something went wrong"});
    }
    ...
    res.status(201).send(newBlog);
        ...
        if(error.message.includes("validation failed"))
        {
            return res.status(422).send({message: 
                Object.values(error.errors).map((item) => item.properties.message)});
        }
        res.status(500).send({message: "Oops! Something went wrong"});
    }
};
...
module.exports = {getBlogs,getBlogById, postNewBlog, deleteBlog, updateBlogById, searchBlogs};
